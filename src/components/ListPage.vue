<template>
  <Card>
    <Row
      ...
      align="middle"
      :class="`${prefixCls}-list-page-head`"
    >
      <Col :class="`${prefixCls}-list-page-title`">{{
        listOption.pageTitle
      }}</Col>
      <Col>
...
export default {
  ...
  methods: {
    ...
    query(pageNo) {
      ...
      if (
        this.queryVal.timeRange &&
        this.queryVal.timeRange.length === 2 &&
        this.queryVal.timeRange[0] !== ""
      ) {
